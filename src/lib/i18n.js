...
const uiMessages = {
  'zh-TW': {
    ...
    serviceIamControls: '服務 IAM 控制',
    serviceIamHint: '如需受控身分、最小權限 RBAC 或高權限審批，請在問卷中補選。',
    azureCliCommands: 'Azure CLI 建置指令',
    cliManualNotes: 'Azure CLI 補充說明',
  ...
  en: {
    ...
    serviceIamControls: 'Service IAM Controls',
    serviceIamHint: 'Select managed identity, least-privilege RBAC, or privileged approval options in the questionnaire when needed.',
    azureCliCommands: 'Azure CLI Build Commands',
    cliManualNotes: 'Azure CLI Follow-up Notes',
  ...
  ja: {
    ...
    serviceIamControls: 'サービス IAM 制御',
    serviceIamHint: 'Managed Identity、最小権限 RBAC、高権限承認が必要な場合はアンケートで選択してください。',
    azureCliCommands: 'Azure CLI 構築コマンド',
    cliManualNotes: 'Azure CLI 補足メモ',
...
const questionTranslations = {
  ...
  serviceIamControls: { en: 'Which service IAM controls are required?', ja: '必要なサービス IAM 制御は何ですか。' },
...
const optionTranslations = {
  ...
  serviceIamControls: {
    'system-assigned-mi': { en: 'Enable system-assigned managed identity for runtime services', ja: '実行時サービスで System-assigned Managed Identity を有効化' },
    'user-assigned-mi': { en: 'Use a user-assigned managed identity shared across services', ja: '複数サービスで User-assigned Managed Identity を共有' },
    'least-privilege-rbac': { en: 'Apply least-privilege RBAC to service principals and automation accounts', ja: 'サービスプリンシパルと自動化アカウントに最小権限 RBAC を適用' },
    'privileged-approval': { en: 'Require approval and time-bound access for privileged roles', ja: '高権限ロールは承認と時間制限付きアクセスを必須化' },
    none: { en: 'No additional service IAM controls for now', ja: '追加のサービス IAM 制御は不要' }
  },
  governanceControls: {
